...
const rollPlayerDice = async (roomId, userId, username) => {
  ...
  try {
    ...
    const gotDoubleTwo = die1 === 2 && die2 === 2;
    
    if (gotDoubleSix || gotDoubleTwo) {
...
const autoRollForTimeout = async (roomId) => {
  ...
  for (const player of game.players) {
    if (!player.isEliminated && !player.hasRolled) {
      ...
      const doubleSix = isDoubleSix(die1, die2);
      const doubleTwo = die1 === 2 && die2 === 2;
      
      if (doubleSix || doubleTwo) {
        player.earnedImmunity = true;
      }
      
      const immunityText = (doubleSix || doubleTwo) ? ' IMMUNITY!' : '';
      rolls.push({ 
        username: player.username, 
        message: `${player.username}: ${formatDiceTags(die1, die2)} = ${total} ${player.isIn ? 'IN!' : 'OUT!'}${immunityText}` 
      });
    }
  }
