...
const startDrawTimer = async (io, roomId, roundNumber = null) => {
  const expiresAt = Date.now() + lowcardService.DRAW_TIMEOUT;
  await lowcardService.setTimer(roomId, 'draw', expiresAt, roundNumber);
  logger.info(`[LowCard] Draw timer started for room ${roomId} round=${roundNumber}, expires in 20s`);
};
...
const processRoundEnd = async (io, roomId, isTimedOut = true) => {
  ...
  if (result.tie) {
    ...
    const tieRound = result.nextRound;
    setTimeout(async () => {
      const game = await lowcardService.getActiveGame(roomId);
      if (game && game.status === 'playing' && game.isTieBreaker && game.currentRound === tieRound) {
        sendBotMessage(io, roomId, `ROUND #${game.currentRound}: Players, !d to DRAW. 20 seconds.`);
        startDrawTimer(io, roomId, game.currentRound);
      }
  ...
  if (result.eliminated) {
    ...
    const nextRound = result.nextRound;
    setTimeout(async () => {
      const game = await lowcardService.getActiveGame(roomId);
      if (game && game.status === 'playing' && game.currentRound === nextRound) {
        sendBotMessage(io, roomId, `ROUND #${nextRound}: Players, !d to DRAW. 20 seconds.`);
        startDrawTimer(io, roomId, nextRound);
      }
    }, lowcardService.COUNTDOWN_DELAY);
  }
};
...
const handleLowcardCommand = async (io, socket, data) => {
  ...
  if (message.startsWith('!start')) {
    ...
    } else if (!result.silent && result.message) {
  ...
  if (message === '!n') {
    const result = await lowcardService.cancelByStarter(roomId, userId, username);
          ...
          if (timer.phase === 'join') {
            ...
            if (result.started) {
              ...
              setTimeout(async () => {
                ...
                if (game && game.status === 'playing' && game.currentRound === 1) {
                  sendBotMessage(io, timer.roomId, `ROUND #1: Players, !d to DRAW. 20 seconds.`);
                  await startDrawTimer(io, timer.roomId, game.currentRound);
                }
              }, lowcardService.COUNTDOWN_DELAY);
            }
            ...
            if (game && game.status === 'playing') {
              if (timer.roundNumber != null && game.currentRound !== timer.roundNumber) {
                logger.info(`[LowCard] Stale draw timer ignored for room ${timer.roomId}: timer round=${timer.roundNumber}, game round=${game.currentRound}`);
                continue;
              }
              await processRoundEnd(io, timer.roomId, true);
            } else {
              logger.info(`[LowCard] Draw timer expired but no active game in room ${timer.roomId}, skipping`);
            }
          }
        }
      }
